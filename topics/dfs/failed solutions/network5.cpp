...
long long lca(vector<vector<long long> >& dfs, vector<bool>& bridges, long long& size, long long a, long long b) {
    if (f[a] > d[b]) return a;
    if (bridges[a]) {
        bridges[a] = false;
        size--;
    }
    return (dfs[a][0] = lca(dfs, bridges, size, dfs[a][0], b));
}

long long kill_up(vector<vector<long long> >& dfs, vector<bool>& bridges, long long& size, long long a, long long b) {
    if (d[b] <= d[a]) return b;
    if (bridges[b]) {
        bridges[b] = false;
        size--;
    }
    return (dfs[b][0] = kill_up(dfs, bridges, size, a, dfs[b][0]));
}
...
void solve(long long n, long long m) {
    ...
    for (long long i = 0; i < a; i++) {
        ...
        b = lca(dfs, bridges, bridges_size, b, c);
        kill_up(dfs, bridges, bridges_size, b, c);
        printf("%lld\n", bridges_size);
    }
}
